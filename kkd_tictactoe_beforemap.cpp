...
class ComputerPlayer
{
    ...
    int m_bestMoveTo = 0;
    int countLinesForMove = 0; //Needed because computer cannot read position from a set tile (Yes, the game board should have been a map...)
...
public:
    ...
    : m_turn(turn), m_board_ptr(board_ptr)
    {
        ...
        improvePosition(m_line1);
        improvePosition(m_line2);
        improvePosition(m_line3);
        improvePosition(m_line4);
        improvePosition(m_line5);
        improvePosition(m_line6);
        improvePosition(m_line7);
        improvePosition(m_line8);
    ...
    void improvePosition(char line[])
    {
        //Count up, so we know which line we are checking
        countLinesForMove++;
        ...
        if (countC == 1 && countX == 0)
        {
            ...
            //     if(line[random] == (random + 48) ) //Adding 48 converts from integer to ASCII value
            //     {
            //         m_bestMoveTo = random;
            //         break;
            //     }
        ...
        if (countC == 1 && countX == 1)
        {
            for (int i = 0; i < 3; i++)
            {
                //Sets bestMoveTo from lines 1, 2 or 3.
                if (line[i] = 'C' && countLinesForMove < 4)
                {
                    m_bestMoveFrom = line[i + countLinesForMove] - 48; //Subtracting 48 converts from ASCII to integer
                }
                //Sets bestMoveTo from lines 4, 5 or 6.
                else if (line[i] = 'C' && countLinesForMove < 7 && countLinesForMove > 3)
                {
                    if (i == 1){m_bestMoveFrom =}

                    m_bestMoveFrom = line[i + ] - 48; //Subtracting 48 converts from ASCII to integer
                }
                else if (line[i] = 'C' && countLinesForMove < 4)
                {
                    m_bestMoveFrom = line[i + countLinesForMove] - 48; //Subtracting 48 converts from ASCII to integer
                }
            }
...
public:
    ...
    {
        m_boardMap.insert(pair<int, char>(1, 'U'));
        m_boardMap.insert(pair<int, char>(2, 'U'));
        m_boardMap.insert(pair<int, char>(3, 'U'));
        m_boardMap.insert(pair<int, char>(4, 'U'));
        m_boardMap.insert(pair<int, char>(5, 'U'));
        m_boardMap.insert(pair<int, char>(6, 'U'));
        m_boardMap.insert(pair<int, char>(7, 'U'));
        m_boardMap.insert(pair<int, char>(8, 'U'));
    ...
    int input;
        ...
        if (m_turn > 3)
        {
            if(getPlayer(m_currentPlayer).isComputer())
            {
                ...
                moveTile(computer.getBestFrom(), computer.getBestFrom(), m_currentPlayer);
            }
            else
            {
            int from;
